...
- `profiles`: defined in `config_defaults.yml` under `.rubocop.yml.profiles`. Two profiles exist: `'on'` (canonical; `NewCops: enable`) and `'off'` (`DisabledByDefault: true`, `NewCops: disable`). The names `cleanups_only`, `strict`, and `hardcore` are backward-compatible aliases that resolve to `'on'`. The `'on'` profile's base EnforcedStyle tunings live inline under `profiles.on.configs` (not a separate YAML anchor -- PDK's `.sync.yml` merge runs before per-file splitting, so a shared anchor would let `.sync.yml` mutate the profile via alias sharing). The block also tunes several PDK / rspec-puppet-convention cops so freshly-scaffolded modules validate clean under `NewCops: enable`; each carries an inline `Description:` rationale -- do NOT remove those, as they prevent future drift and accidental re-enabling.
