...
PDK builds the configuration of the module by reading a set of default configuration from the file `config_defaults.yml` and merging it with the contents of `.sync.yml` from module's root directory, if exists. Top-level keys of the resulting hash correspond to a target file or represent a global configuration. Global configuration will be merged with the hash of configuration values within the target file key. This allows a module developer to override/amend the configuration by putting new values into `.sync.yml`. A knockout prefix, applied to elements within `.sync.yml`, allows to [remove values from the default set](#removing-default-configuration-values). Templates are referring their configuration values via `@configs` hash. The data for a target file may include `delete: true` or `unmanaged: true` in order to have the particular file removed or left unmanaged, respectively.
